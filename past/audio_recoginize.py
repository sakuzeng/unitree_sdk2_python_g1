...
# 音频缓冲区
original_audio_buffer = deque(maxlen=16000 * 30)  # 原始音频缓冲区
...
class DeepFilterNetProcessor:
    ...
    def process_audio(self, audio_np):
        ...
        try:
            # 转换为 float32 格式 (-1.0 到 1.0) 并添加批次和通道维度
...
class VoiceAssistant:
    ...
    def process_audio_frame(self, audio_data):
        """处理音频帧"""
        global original_audio_buffer, is_listening
        
        frame_duration = 10  # 10ms帧
        frame_size = int(FRAME_RATE * frame_duration / 1000)
        frames = [audio_data[i:i + frame_size * SAMPLE_WIDTH] for i in range(0, len(audio_data), frame_size * SAMPLE_WIDTH)]
        ...
        for frame in frames:
            ...
            # 直接添加到缓冲区（不依赖VAD触发）
            if is_listening:
                original_audio_buffer.extend(frame_np)
    ...
    def process_complete_speech(self):
        ...
        if not original_audio_buffer:
            print("⚠️ 没有检测到有效语音")
            self.speak("抱歉，没有收到语音输入")
            return
            
        # 转换为numpy数组
        original_audio = np.array(list(original_audio_buffer), dtype=np.int16)
        ...
        # 保存音频数据
        denoised_wav_path = self.save_audio_session(original_audio, denoised_audio, session_counter)
    ...
    def start_conversation(self):
        """开始对话"""
        global audio_receiver_running, audio_receiver_thread, is_listening
...
def signal_handler(signum, frame):
    """信号处理"""
    global audio_receiver_running, is_listening
    print("\n🛑 接收到退出信号，正在关闭...")
    audio_receiver_running = False
    is_listening = False
    sys.exit(0)
...
def main():
    ...
    try:
        ...
        print("💡 功能说明：")
        print("   1. 按回车开始录音")
        print("   2. 使用 DeepFilterNet 进行深度学习降噪")
        print("   3. 保存原始和降噪后的音频文件")
        print("   4. 对降噪后的音频进行语音识别")
        print("   5. 语音输出识别结果")
